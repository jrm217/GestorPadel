...
public interface ReservaRepository extends JpaRepository<Reserva, Long> {
    ...
    Optional<Reserva> findByPistaAndHorario(String pista, LocalTime horaInicio, LocalTime horaFin);
    
    @Query("SELECT r FROM Reserva r WHERE r.usuario = :usuario AND r.horaInicio = :inicio AND r.horaFin = :fin")
    Optional<Reserva> findByUsuarioAndHorario(@Param("usuario") Usuario usuario, @Param("inicio") LocalTime inicio, @Param("fin") LocalTime fin);
